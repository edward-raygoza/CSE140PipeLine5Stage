...
//pipeline registers
//signals to store in IF/ID pipeline stage
struct IF_ID	
{
	int pc_4;
...
//function to identify the RAW (Read after write) data hazard
void forwarding()
{
	if (ie_mem_reg.reg_write && ie_mem_reg.reg_dst != 0
		&& ie_mem_reg.reg_dst == id_ie_reg.rs)
	{
		forwardOne = 2;
		forwardTwo = 0;
	}
	else if (ie_mem_reg.reg_write && ie_mem_reg.reg_dst != 0
		&& ie_mem_reg.reg_dst == id_ie_reg.rt)
	{
		forwardOne = 0;
		forwardTwo = 2;
		
	}
	 else if (mem_wb_reg.reg_write && mem_wb_reg.reg_dst != 0
		&& mem_wb_reg.reg_dst == id_ie_reg.rs)
	{
		forwardOne = 1;
		forwardTwo = 0;
	}
	else if (mem_wb_reg.reg_write && mem_wb_reg.reg_dst != 0
		&& mem_wb_reg.reg_dst == id_ie_reg.rt)
	{
		forwardOne = 0;
		forwardTwo = 1;
	}
...
void execution()
{
	...
	case 0:
		...
		break;
	case 1:
		alu_op1 = mem_wb_reg.alu_out;
		break;
	case 2:
		alu_op1 = ie_mem_reg.alu_out;
		break;
	}
	switch (forwardTwo)
	{
	case 0:
		alu_op2 = id_ie_reg.alu_src_mux ? id_ie_reg.immediate : id_ie_reg.read_data2;
		break;
	case 1:
		alu_op2 = ie_mem_reg.alu_out;
		break;
	case 2:
		alu_op2 = mem_wb_reg.alu_out;
		break;
